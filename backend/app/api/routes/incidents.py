...
async def autofix_incident(incident_id: int, db: AsyncSession = Depends(get_db)):
    ...
    project_id = getattr(settings, "GITLAB_PROJECT_ID", "82734152")

    # Fetch actual .gitlab-ci.yml so Gemini has real context
    current_yml_content = ""
    try:
        ...
        logger.warning("Could not fetch .gitlab-ci.yml — using fallback: %s", exc)
        current_yml_content = (
            "stages:\n  - build\n  - test\n  - deploy\n\n"
            "build-job:\n  stage: build\n  script:\n    - echo 'Building...'\n"
        )
    ...
    try:
        ...
        mr = await create_fix_mr(
            ...
            description=(
                ...
                f"*Generated by OpsPilot AI x Gemini 2.5 Flash x Google ADK*"
            ),
        )
