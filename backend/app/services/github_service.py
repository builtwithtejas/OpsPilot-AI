...
def get_workflow_runs(limit: int = 10) -> list[dict]:
    try:
        ...
        for run in repo.get_workflow_runs()[:limit]:
            # run_number is always an int from GitHub API; guard anyway
            run_number = getattr(run, "run_number", None)
            if run_number is None:
                run_number = getattr(run, "id", 0)

            # created_at from PyGithub is a datetime object — convert to ISO string
...
def build_analytics(runs: list[dict]) -> dict:
    ...
    for run in runs:
        ...
        # Robust ISO datetime parsing — handles +00:00 and Z suffixes
        try:
            ...
            hour = created_at_raw[11:16] if len(created_at_raw) >= 16 else "??:00"
    ...
    # Suggested commands based on actual failure patterns
    if failed > 0:
        suggested_commands = [
    ...
    analysis = (
        f"{failed} failed deployment(s) detected out of {total} total runs "
